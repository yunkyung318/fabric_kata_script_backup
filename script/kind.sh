...
function kind_create() {
  ...
  kind create cluster --name $CLUSTER_NAME --config=kind_crio.yaml
  for node in $(kind get nodes);
  do
    docker exec "$node" sysctl net.ipv4.conf.all.route_localnet=1;
  done
  
  for node in $(kind get nodes --name crio);
  do
    docker cp crio $node:/usr/bin/crio
    docker exec $node systemctl restart crio
  done
